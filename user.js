...
const translateWidgetEl = $("userTranslateWidget");
const translateToggleEl = $("translateToggle");
const translateTrayEl = $("translateTray");
const translateLanguageEl = $("translateLanguage");
...
function show(sectionId) {
  ...
  if (translateWidgetEl) translateWidgetEl.classList.toggle("hidden", sectionId === "auth" || !token);
...
function populateTranslateLanguages() {
  if (!translateLanguageEl || translateLanguageEl.dataset.ready === "true") return;
  for (const [code, name] of TRANSLATE_LANGUAGES) {
    const option = document.createElement("option");
    option.value = code;
    option.textContent = name;
    translateLanguageEl.appendChild(option);
  }
  translateLanguageEl.dataset.ready = "true";
  const saved = localStorage.getItem("aegisTranslateLanguage") || "";
  if (saved) translateLanguageEl.value = saved;
}
...
function initTranslateWidget() {
  ...
  if (translateToggleEl) translateToggleEl.onclick = () => translateTrayEl && translateTrayEl.classList.toggle("hidden");
  if (translateLanguageEl) translateLanguageEl.onchange = () => applySelectedTranslation(translateLanguageEl.value);
}

window.initGoogleTranslate = function initGoogleTranslate() {
  if (window.google && window.google.translate && document.getElementById("googleTranslateElement")) {
